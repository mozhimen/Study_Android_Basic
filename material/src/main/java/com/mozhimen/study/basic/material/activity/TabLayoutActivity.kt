...
import com.mozhimen.study.basic.material.databinding.ActivityTabLayoutBinding
import com.mozhimen.study.basic.material.dialog.CompanyDialog
import com.mozhimen.study.basic.material.fragment.Fragment1
import com.mozhimen.study.basic.material.fragment.Fragment2
import com.mozhimen.study.basic.material.fragment.Fragment3
...
class TabLayoutActivity : ToolbarActivity<ActivityTabLayoutBinding>() {
    ...
    override fun initMaterialToolbar(materialToolbar: MaterialToolbar?) {
        ...
        materialToolbar?.setTitle(R.string.tab_layout)
    }
    ...
    private fun initViewPager() {
        //viewpager设置adapter
        vb.viewPager.adapter = SimpleFragmentPagerAdapter(supportFragmentManager)
        vb.viewPager.addOnPageChangeListener(object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                vb.tabLayout8.getTabAt(position)?.select()
            }
    ...
    private fun initTabLayout1() {
        ...
        vb.tabLayout1.setupWithViewPager(vb.viewPager)
        //设置默认选中
        vb.tabLayout1.getTabAt(defaultIndex)?.select()
    }
    ...
    private fun initTabLayout2() {
        vb.tabLayout2.setupWithViewPager(vb.viewPager)
        for (index in 0..vb.tabLayout2.tabCount) {
            vb.tabLayout2.getTabAt(index)?.setIcon(R.mipmap.ic_launcher_round)
        }
    }
    ...
    private fun initTabLayout3() {
        vb.tabLayout3.setupWithViewPager(vb.viewPager)
    }
    ...
    private fun initTabLayout4() {
        vb.tabLayout4.setupWithViewPager(vb.viewPager)
        //设置 分割线
        for (index in 0..vb.tabLayout4.tabCount) {
            val linearLayout = vb.tabLayout4.getChildAt(index) as? LinearLayout
            linearLayout?.let {
    ...
    private fun initTabLayout5() {
        vb.tabLayout5.setupWithViewPager(vb.viewPager)

        // 数字
        vb.tabLayout5.getTabAt(defaultIndex)?.let { tab ->
        ...
        vb.tabLayout5.getTabAt(1)?.let { tab ->
            tab.orCreateBadge.backgroundColor = ContextCompat.getColor(this, com.mozhimen.uicorek.R.color.cok_oran_ff511a)
        }
    }
    ...
    private fun initTabLayout6() {
        vb.tabLayout6.setupWithViewPager(vb.viewPager)
        vb.tabLayout6.addOnTabSelectedListener(object : OnTabSelectedListener {
    ...
    private fun initTabLayout7() {
        companyMap.forEach {
            ...
            vb.tabLayout7.addTab(tab)
        }

        // mBinding.tabLayout7.setOnScrollChangeListener() // min api 23 (6.0)
        // 适配 5.0  滑动过程中判断右侧小红点是否需要显示
        vb.tabLayout7.viewTreeObserver.addOnScrollChangedListener {
            vb.vArrowDot.visibility = if (isShowDot()) View.VISIBLE else View.INVISIBLE
        }

        vb.ivArrow.setOnClickListener {
            CompanyDialog(companyMap, vb.tabLayout7.selectedTabPosition).show(supportFragmentManager, "CompanyDialog")
        }
    }
    ...
    override fun onResume() {
        super.onResume()
        // 初始化判断右侧小红点是否需要显示
        vb.tabLayout7.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                vb.vArrowDot.visibility = if (isShowDot()) View.VISIBLE else View.INVISIBLE
                vb.tabLayout7.viewTreeObserver.removeOnGlobalLayoutListener(this)
            }
        })
    }
    ...
    private fun isShowDot(): Boolean {
        ...
        companyMap.keys.forEachIndexed { index, _ ->
            vb.tabLayout7.getTabAt(index)?.let { tab ->
    ...
    private fun initTabLayout8() {
        ...
        animMap.keys.forEach { s ->
            ...
            textView.text = s
            tab.customView = view
            vb.tabLayout8.addTab(tab)
        }

        val defaultTab = vb.tabLayout8.getTabAt(0)
        defaultTab?.select()
        defaultTab?.setSelected()

        vb.tabLayout8.addOnTabSelectedListener(object : OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab?) {
                tab?.setSelected()
                tab?.let { vb.viewPager.currentItem = it.position }
            }
